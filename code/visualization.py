...
def cm_plot(df, drop_attributes, saved_plot):
    """
    This function is to draw correlation matrix for the input dataframe
    :df: input dataframe
    ...
    sns.heatmap(df.drop(drop_attributes, axis=1).corr(), square=True, cmap='Blues')
...
def neighborhood_rooms_distribution(df, top, saved_plot):
    ...
    clr1 = (
        'brown', 'darksalmon', 'orange', 'hotpink', 'cadetblue', 'purple', 'red', 'gold', 'forestgreen', 'blue', 'plum')
    qx = type_dict.plot(kind='pie', y='counts', labels=groups, colors=clr1, autopct='%1.1f%%', pctdistance=0.9,
                        radius=1.2, ax=ax[1])
...
def neighborhood_sort_by_mean_price(df, top):
    ...
    data_top_prices_by_neighbourhood = data.groupby('neighbourhood').agg({'price': 'mean'}).sort_values(
        'price').reset_index()
...
def interactive_map(df, attribute, hover_data):
    ...
    fig.update_layout(
        title_text="Classify listed rooms by " + attribute + " in NYC<br>(Click legend to toggle borough)", height=400)
    fig.show()
    plt.close()
